...
const FIELD_INSTRUCTIONS: Record<CopyField, string> = {
  ...
  campaign_writer: `Output ONLY writer instructions for an AI that drafts LinkedIn messages: tone, length cap (prefer short DMs), must-mention, avoid, CTA style. Explicitly say: never append a personal name signature. Bullet lines or short paragraphs. 40–250 words.`,
  global_writer: `Output ONLY global voice instructions reused across campaigns: tone, positioning, must-mention, avoid. Explicitly say: never append a personal name or letter sign-off. 40–200 words.`,
  ...
  post_hook: `Output ONLY a LinkedIn post opening hook (1–3 sentences). Plain text only (no markdown). Punchy, concrete. No hashtags unless brief asks. No name signature. Match the requested post language.`,
  post_body: `Output ONLY the LinkedIn post body (after the hook, do not repeat the hook). ${LINKEDIN_POST_COPY_RULES} Match the requested post language. Prefer ~800–1300 characters unless brief asks shorter or longer.`,
